describe('Mailing List Signup', () => {
  ...
  it('has correct call to action text', () => {
    cy.get('[data-cy=mailing-list-cta').should('contain', 'Sign up for our mailing list to receive important QHacks announcements!');
  });

  it('has the right placeholder in email input', () => {
    cy.get('[data-cy=mailing-list-input').should('have.attr', 'placeholder', 'Enter your email address');
  });

  it('updates text when typing in email input', () => {
    cy.get('[data-cy=mailing-list-input').type('test@test.com').should('have.value', 'test@test.com');
  });
