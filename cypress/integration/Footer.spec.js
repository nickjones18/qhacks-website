describe('Footer', () => {
  ...
  it('provides link to MLH Code of Conduct', () => {
    cy.get('[data-cy=footer-mlh-code-of-conduct-link]')
      ...
      .should('have.attr', 'href', 'https://static.mlh.io/docs/mlh-code-of-conduct.pdf');
  });
  ...
  it('has correct year for copyright notice', () => {
    cy.get('[data-cy=footer-copyright-notice]').should('have.length', 2).each(($el, index, $list) => {
      expect($el).to.have.text(`Copyright © ${Cypress.moment().year()} QHacks`);
    });
  });
